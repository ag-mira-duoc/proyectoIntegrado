...
# Configuración adicional de PostgreSQL
# Añadir la opción `search_path` solo cuando el motor sea PostgreSQL.
engine = DATABASES['default'].get('ENGINE', '')
if engine and 'postgres' in engine:
    DATABASES['default'].setdefault('OPTIONS', {})
    DATABASES['default']['OPTIONS'].update({'options': '-c search_path=public'})
elif engine and 'sqlite' in engine:
    # Normalize sqlite settings to avoid invalid connection kwargs
    sqlite_name = DATABASES['default'].get('NAME') or str(BASE_DIR / 'db.sqlite3')
    DATABASES['default'] = {
